...
<%@ page import="com.homeelectronics.model.Order" %> <%-- Import your Order model --%>
...
<body>
    ...
    <div class="offcanvas offcanvas-end pb-sm-2 px-sm-2" id="orderDetails" tabindex="-1" aria-labelledby="orderDetailsLabel" style="width: 500px">
        ...
        <div class="offcanvas-header align-items-start py-3 pt-lg-4">
            <div>
                <h4 class="offcanvas-title mb-1" id="orderDetailsLabel">Order #<span id="offcanvas-order-id"></span></h4>
                <span class="d-flex align-items-center fs-sm fw-medium text-body-emphasis" id="offcanvas-order-status">
                </span>
            </div>
        ...
        <div class="offcanvas-body d-flex flex-column gap-4 pt-2 pb-3">
            ...
            <div class="d-flex flex-column gap-3" id="offcanvas-item-list">
                <div class="d-flex justify-content-center p-5" id="offcanvas-loader">
            ...
            <div class="border-top pt-4">
                <ul class="list-unstyled fs-sm mb-4">
                    ...
                    <li class="d-flex justify-content-between">
                        Payment method:
                        <span class="text-body-emphasis fw-medium text-end ms-2" id="offcanvas-payment-method">Cash on delivery</span>
                    </li>
                </ul>
            </div>
            ...
            <div class="d-flex align-items-center justify-content-between fs-sm border-top pt-4">
                Estimated total:
                <span class="h5 text-end ms-2 mb-0" id="offcanvas-total">₹0.00</span>
            </div>
        </div>
  ...
  <a class="nav-link d-block mb-0" href="home-electronics.jsp">
                    ...
                    <div class="ps-lg-3 ps-xl-0">
                        ...
                        <div data-filter-list="{&quot;listClass&quot;: &quot;orders-list&quot;, &quot;sortClass&quot;: &quot;orders-sort&quot;, &quot;valueNames&quot;: [&quot;date&quot;, &quot;total&quot;]}">
                            <table class="table align-middle fs-sm text-nowrap">
                                ...
                                <%-- === DYNAMIC ORDER LISTING START === --%>
                                <c:choose>
                                    <c:when test="${not empty orders}">
                                        <c:forEach var="order" items="${orders}">
                                            <tr>
                                                <td class="fw-medium pt-2 pb-3 py-md-2 ps-0">
                                                        <%-- Link to open order details offcanvas (needs JS) --%>
                                                    <a class="d-inline-block animate-underline text-body-emphasis text-decoration-none py-2 order-details-link"
                                                       href="#orderDetails"
                                                       data-bs-toggle="offcanvas"
                                                       data-order-id="${order.id}" <%-- Pass order ID --%>
                                                    ...
                                                    <ul class="list-unstyled fw-normal text-body m-0 d-md-none">
                                                        <li><fmt:formatDate value="${order.orderDate}" pattern="MMM d, yyyy"/></li>
                                                        ...
                                                        <li class="fw-medium text-body-emphasis">
                                                            ₹<fmt:formatNumber value="${order.totalAmount}" type="number" minFractionDigits="2" maxFractionDigits="2"/> <%-- INR Symbol --%>
                                                        </li>
                                                    </ul>
                                                </td>
                                                ...
                                                <td class="fw-medium py-3 d-none d-md-table-cell">
                                                    ...
                                                    <span class="date d-none"><fmt:formatDate value="${order.orderDate}" pattern="yy-MM-dd"/></span>
                                                </td>
                                                <td class="fw-medium py-3 d-none d-md-table-cell">
                                    ...
                                    <c:otherwise>
                                        <span class="bg-secondary rounded-circle p-1 me-2"></span> ${order.paymentStatus}
                                                ...
                                                <td class="fw-medium py-3 d-none d-md-table-cell">
                                                        <%-- Formatted Total --%>
                                                    ₹<fmt:formatNumber value="${order.totalAmount}" type="number" minFractionDigits="2" maxFractionDigits="2"/> <%-- INR Symbol --%>
                                                        <%-- Hidden total for sorting --%>
                                                    <span class="total d-none">${order.totalAmount * 100}</span> <%-- Use paisa/cents for sorting --%>
                                                </td>
                                                <td class="py-3 pe-0">
                                                        <%-- Thumbnails/Link (Simplified - show placeholder) --%>
                                                    <span class="d-flex align-items-center justify-content-end position-relative gap-1 gap-sm-2 ms-n2 ms-sm-0">


                                <a class="btn btn-icon btn-ghost btn-secondary stretched-link border-0 order-details-link"
                                   href="#orderDetails"
                                   data-bs-toggle="offcanvas"
                                   data-order-id="${order.id}" <%-- Pass order ID again --%>
                                   aria-controls="orderDetails"
                                   aria-label="Show order details">
    ...
    <script src="assets/js/account-orders.js" data-context-path="${pageContext.request.contextPath}" defer></script>
